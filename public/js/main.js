$(function () {
    ...
    $.get('/get_chatters', function (response) {
        $('.chat-info').text("There are currently" + response.length + "people in the chat writing ");
        chatters_count = response.length;
    });
//
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status === 'OK') {
                    ...
                    alert("Sorry but the username already exists,please choose another one and try again!");
                    $('#username').val('').focus();
                }
            }
        });
